...
const { AgentRegistry, resolveBinding, codexThreadIdFromMeta, claudePidFromSocketPath } = require('../out/test/agent-registry.js');
...
test('sessions are ordered by recency', () => {
    const registry = new AgentRegistry();
    registry.register(session('old', 1)); registry.register(session('new', 2));
    assert.deepEqual(registry.list().map(s => s.sessionId), ['new', 'old']);
});
