...
- [x] Emitted prompts treat a comment like a normal chat message: `renderReviewPrompt` now carries only the comment payload plus one closing line naming the thread tools ("Reply in each thread with `diffReview_replyToComment` (threadId and text); `diffReview_resolveComment` marks it resolved." — MCP names on the direct-delivery path). The five-step policy survives only in `/address-diff-review`; `prosePolicy`, `ThreadRef`, and `PolicyOptions` are deleted, and the slash-command bodies stayed byte-identical so no marker version bump was needed.
