...
import { addressSteps, sectionedPolicy, prosePolicy, MCP_TOOLS, LM_TOOLS, type PolicyOptions } from './review-policy';
import { renderBody, MARKER } from './slash-commands';

const MCP: PolicyOptions = { tools: MCP_TOOLS, threadRef: 'listed' };
const LM: PolicyOptions = { tools: LM_TOOLS, threadRef: 'inline' };

test('addressSteps: the five steps, in order', () => {
  assert.deepStrictEqual(
    addressSteps(MCP).map((s) => s.heading),
    [
...
test('addressSteps: tool names come from the caller, not the module', () => {
  const mcp = addressSteps(MCP)
    .map((s) => s.body)
    .join('\n');
  const lm = addressSteps(LM)
    .map((s) => s.body)
    .join('\n');
