...
import { normalizeAgentLabel, claudeSessionLabel, codexSessionLabel } from './agent-label';
...
test('Claude label comes from the matching private session registry entry', () => {
...
test('Codex label prefers the database result and rejects malformed IDs', () => {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'diff-review-label-test-'));
  onTestFinished(() => fs.rmSync(home, { recursive: true, force: true }));
  ...
  const fakeSqlite = (
    ...
    _options: Parameters<typeof import('node:child_process').execFileSync>[2],
  ) => {
    calls++;
