...
import {
  AgentRegistry,
  resolveBinding,
  codexThreadIdFromMeta,
  claudePidFromSocketPath,
  type AgentSession,
} from './agent-registry';
const session = (id: string, recency: number): Omit<AgentSession, 'registeredAt' | 'lastSeenAt'> => ({
  agent: 'codex',
  sessionId: id,
  label: id,
  cwd: '/work',
...
test('registration is idempotent and preserves registeredAt', () => {
  ...
  const first = registry.register(session('one', 1), new Date('2026-01-01'));
  const second = registry.register(session('one', 2), new Date('2026-01-02'));
...
test('sessions are ordered by recency', () => {
  const registry = new AgentRegistry();
  registry.register(session('old', 1));
  registry.register(session('new', 2));
  assert.deepEqual(
    registry.list().map((s) => s.sessionId),
    ['new', 'old'],
  );
});

test('binding resolves explicitly, silently for one, and not for many', () => {
  const one = session('one', 1),
    two = session('two', 2);
  const registry = new AgentRegistry();
  const storedOne = registry.register(one);
  const storedTwo = registry.register(two);
  assert.equal(resolveBinding([storedOne], undefined), storedOne);
  assert.equal(resolveBinding([storedOne, storedTwo], 'one'), storedOne);
  assert.equal(resolveBinding([storedOne, storedTwo], 'gone'), undefined);
});
