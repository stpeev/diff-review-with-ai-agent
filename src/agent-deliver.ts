...
export type DeliverableAgentSession = Pick<
  AgentSession,
  'agent' | 'sessionId' | 'label' | 'cwd' | 'socketPath' | 'token' | 'pid'
>;
...
export function resolveCodexBinary(envPath = process.env.PATH ?? '', home = os.homedir()): string | undefined {
  ...
  for (const root of roots) {
    ...
    try {
      versions = fs
        .readdirSync(root)
        .filter((x) => /^openai\.chatgpt-/.test(x))
        .sort()
        .reverse();
    } catch {
      continue;
 ...
 */
export function deliverToClaude(session: DeliverableAgentSession, message: string, timeoutMs = 5000): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const finish = (error?: Error) => {
      ...
      if (dropped) {
        reject(
          new Error(
            `Claude rejected the message${typeof dropped.drop_reason === 'string' ? `: ${dropped.drop_reason}` : ''}`,
          ),
        );
        return;
      }
      ...
      resolve(
        acknowledged
          ? 'accepted by Claude messaging socket'
          : 'written to Claude messaging socket (no protocol acknowledgement)',
      );
    };
...
export function deliverToSession(session: DeliverableAgentSession, message: string): Promise<string> {
  ...
  return new Promise((resolve, reject) =>
    execFile(
      binary,
      ['queue', '--thread', session.sessionId, '--message', message],
      { timeout: 15000 },
