...
export class AgentRegistry {
  ...
  get(sessionId: string | undefined): AgentSession | undefined {
    return sessionId ? this.sessions.get(sessionId) : undefined;
  }
  list(): AgentSession[] {
    return [...this.sessions.values()].sort(
      (a, b) => (b.recency ?? Date.parse(b.lastSeenAt)) - (a.recency ?? Date.parse(a.lastSeenAt)),
    );
  }
}

export function resolveBinding<Session extends AgentSession>(
  sessions: Session[],
...
export function claudePidFromSocketPath(socketPath: string | undefined): number | undefined {
  ...
  const match =
    /^(?:\/(?:private\/)?tmp\/cc-socks(?:-\d+)?|\/run\/user\/\d+\/cc-socks|\/data\/data\/com\.termux\/files\/usr\/tmp\/cc-socks(?:-\d+)?)\/(\d+)\.sock$/.exec(
      socketPath,
    );
