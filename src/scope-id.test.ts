...
test('deepestScopeForFile keeps mixed git/non-git roots separate', () => {
  ...
  const git = deepestScopeForFile(candidates, '/Users/dev/ws/git-repo/a.ts');
  const plain = deepestScopeForFile(candidates, '/Users/dev/ws/plain-folder/b.ts');
  assert.ok(git);
  assert.ok(plain);
  assert.strictEqual(git.scopeId, 'repo:git-one');
  assert.strictEqual(plain.scopeId, 'folder:plain-one');
});

test('deepestScopeForFile retains caller metadata on the selected candidate', () => {
  const folder = { path: '/Users/dev/ws/nested' };
  const match = deepestScopeForFile(
    [{ scopeId: 'folder:nested', folderRealPath: folder.path, folder }],
    '/Users/dev/ws/nested/file.ts',
  );

  assert.strictEqual(match?.folder, folder);
});
