...
import {
  __escapeTomlMultilineStringForTest,
...
function targetFor(targets: SlashCommandTarget[], id: string): SlashCommandTarget {
  return required(
...
function fileFor(target: SlashCommandTarget, command: CommandId) {
  return required(
    target.files.find((file) => file.command === command),
...
test('renderBody: every wrapper carries the marker for its command', () => {
  for (const kind of ['claude-md', 'codex-md', 'gemini-toml', 'vscode-prompt'] as CommandKind[]) {
...
test('renderBody: the instructional Markdown is identical across all four wrappers', () => {
  for (const command of ['perform', 'address', 'register', 'unregister'] as CommandId[]) {
...
test('discoverSlashCommands: Gemini CLI row appears from ~/.gemini alone', () => {
  ...
  const targets = discoverSlashCommands({ home, platform: 'linux' });
  assert.ok(
    targets.find((t) => t.id === 'gemini'),
    'expected a gemini row from the home dir alone',
  );
});
...
test('discoverSlashCommands: discovery never creates the command directory', () => {
  const home = tmpHome();
  fs.mkdirSync(path.join(home, '.codex'), { recursive: true });
  const codex = targetFor(discoverSlashCommands({ home, platform: 'linux' }), 'codex');
...
test('discoverSlashCommands: a fully current file is status current and writable', () => {
  ...
  fs.writeFileSync(path.join(dir, 'perform-diff-review.md'), renderBody('claude-md', 'perform'));
  const targets = discoverSlashCommands({ home, platform: 'linux' });
  const claude = targetFor(targets, 'claude');
  const perform = fileFor(claude, 'perform');
...
test('discoverSlashCommands: a stale file with our marker is writable', () => {
  ...
  const targets = discoverSlashCommands({ home, platform: 'linux' });
  const perform = fileFor(targetFor(targets, 'claude'), 'perform');
...
test('discoverSlashCommands: a stale file without our marker is not writable', () => {
  ...
  assert.match(required(perform.reason), /not written by Diff Review/);
});
...
test('discoverSlashCommands: row status is the worse of its files (missing wins)', () => {
  ...
  const claude = targetFor(discoverSlashCommands({ home, platform: 'linux' }), 'claude');
  assert.strictEqual(claude.status, 'missing');
});
...
test('discoverSlashCommands: row status is stale when one file is stale and the others current', () => {
  ...
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'perform-diff-review.md'), renderBody('claude-md', 'perform'));
  fs.writeFileSync(path.join(dir, 'address-diff-review.md'), `${MARKER.address}\nold\n`);
  fs.writeFileSync(path.join(dir, 'register-for-diff-review-send.md'), renderBody('claude-md', 'register'));
  fs.writeFileSync(path.join(dir, 'unregister-for-diff-review-send.md'), renderBody('claude-md', 'unregister'));
  const claude = targetFor(discoverSlashCommands({ home, platform: 'linux' }), 'claude');
...
test('discoverSlashCommands: row status is current only when all files are', () => {
  ...
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'perform-diff-review.md'), renderBody('claude-md', 'perform'));
  fs.writeFileSync(path.join(dir, 'address-diff-review.md'), renderBody('claude-md', 'address'));
  fs.writeFileSync(path.join(dir, 'register-for-diff-review-send.md'), renderBody('claude-md', 'register'));
  fs.writeFileSync(path.join(dir, 'unregister-for-diff-review-send.md'), renderBody('claude-md', 'unregister'));
  const claude = targetFor(discoverSlashCommands({ home, platform: 'linux' }), 'claude');
...
test('renderClipboard: includes the body and a save-this-to line naming the path', () => {
  ...
  assert.ok(clip.includes(MARKER.perform));
...
test('install: skips a file that is already current', () => {
  ...
  assert.deepStrictEqual(
    outcome.written.map((w) => w.command),
    ['address', 'register', 'unregister'],
  );
});
...
test('install: backs up a stale-with-marker file before overwriting it', () => {
  ...
  const target = targetFor(discoverSlashCommands({ home, platform: 'linux' }), 'claude');
  const outcome = install(target);
  const result = required(outcome.written.find((w) => w.command === 'perform'));
  assert.ok(result.backup);
  assert.strictEqual(fs.readFileSync(result.backup, 'utf-8'), `${MARKER.perform}\nold body\n`);
  assert.strictEqual(
    fs.readFileSync(path.join(dir, 'perform-diff-review.md'), 'utf-8'),
    renderBody('claude-md', 'perform'),
  );
...
test('install: never touches a stale-without-marker file', () => {
  ...
  const outcome = install(target);
  assert.strictEqual(
    outcome.written.find((w) => w.command === 'perform'),
    undefined,
  );
...
test('install: creates the parent directory when it does not exist yet', () => {
  ...
  const target: SlashCommandTarget = {
    ...
    files: [
      {
        command: 'perform',
        filePath: path.join(dir, 'perform-diff-review.md'),
      ...
      {
        command: 'address',
        filePath: path.join(dir, 'address-diff-review.md'),
        invocation: '/address-diff-review',
        status: 'missing',
        writable: true,
      },
      {
        command: 'register',
        filePath: path.join(dir, 'register-for-diff-review-send.md'),
        invocation: '/register-for-diff-review-send',
        status: 'missing',
        writable: true,
      },
      {
        command: 'unregister',
        filePath: path.join(dir, 'unregister-for-diff-review-send.md'),
        invocation: '/unregister-for-diff-review-send',
        status: 'missing',
        writable: true,
      },
    ],
