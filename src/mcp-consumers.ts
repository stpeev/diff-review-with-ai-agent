 ...
 * host. See test/mcp-consumers.test.ts.
 */
...
export function samePath(a: string, b: string, home = os.homedir()): boolean {
  const norm = (p: string) => {
    ...
    try {
      ...
      /* not on disk yet; the literal path is the best we have */
    }
...
function classifyEntry(entry: any, launcher: string, home: string): Inspection {
  ...
  const isCurrent = entry?.command === 'node' && args.length === 1 && samePath(String(args[0]), launcher, home);
  return isCurrent
...
export function writeVsCodeJson(text: string | null, launcher: string): string {
  ...
  const key = parsed && typeof parsed === 'object' && parsed.mcpServers && !parsed.servers ? 'mcpServers' : 'servers';
...
function findCodexBlock(lines: string[]): { start: number; end: number } | null {
  ...
  for (let i = start + 1; i < lines.length; i++) {
    ...
    if (header && !header.startsWith(`mcp_servers.${SERVER_NAME}.`)) {
      end = i;
      break;
    }
  }
...
export function discoverConsumers(env: DiscoveryEnv = {}): McpConsumerTarget[] {
  ...
  for (const app of VSCODE_APPS) {
    ...
    const config = app.id === 'cursor' && fs.existsSync(cursorConfig) ? cursorConfig : path.join(root, 'mcp.json');

    targets.push(
      targetFrom(app.id, app.label, 'vscode-json', config, (text) => inspectVsCodeJson(text, launcher, home)),
    );
  ...
  if (fs.existsSync(codexHome)) {
    targets.push(
      targetFrom('codex', 'Codex CLI', 'codex-toml', path.join(codexHome, 'config.toml'), (text) =>
        inspectCodexToml(text, launcher, home),
      ),
    );
  }
  ...
  if (fs.existsSync(claudeConfig) || fs.existsSync(path.join(home, '.claude'))) {
    targets.push(
      targetFrom('claude', 'Claude Code', 'claude-json', claudeConfig, (text) =>
        inspectClaudeJson(text, launcher, home),
      ),
    );
  }
...
export function renderSnippet(target: McpConsumerTarget, launcher = LAUNCHER_FILE, home = os.homedir()): string {
  switch (target.kind) {
    case 'codex-toml':
      return codexSnippet(launcher);
    case 'claude-json':
      return claudeSnippet(launcher, home);
    case 'vscode-json':
      return vsCodeSnippet(launcher);
  }
}
...
export function register(target: McpConsumerTarget, launcher = LAUNCHER_FILE): WriteResult {
  ...
  const transform =
    target.kind === 'codex-toml' ? writeCodexToml : target.kind === 'claude-json' ? writeClaudeJson : writeVsCodeJson;
