...
function registeredTools() {
  ...
  registerMcpTools(server, registrar, vi.fn());
  return { callbacks, registrar };
}
...
test('routes Codex registration metadata through the session registrar', async () => {
  ...
  expect(registrar.registerCodexSession).toHaveBeenCalledWith('thread-123456', '  Topic review  ');
